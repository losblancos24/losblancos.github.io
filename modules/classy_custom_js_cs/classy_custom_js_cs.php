...
class Classy_custom_js_cs extends Module
{
    ...
    public function uninstall()
    {
        ...
        Configuration::deleteByName('CLASSY_HEAD_CUSTOM_JS');
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'col' => 5,
                        'type' => 'textarea',
                        'desc' => $this->l('JS Load in front js file'),
                        'name' => 'CLASSY_CUSTOM_JS',
                        'label' => $this->l('JS In Footer'),
                    ),
                    array(
                        'col' => 5,
                        'type' => 'textarea',
                        'desc' => $this->l('Load js file with Script Tag'),
                        'name' => 'CLASSY_HEAD_CUSTOM_JS',
                        'label' => $this->l('JS In Head'),
                    ...
                    array(
                        ...
                        'type' => 'textarea',
                        'desc' => $this->l('Load js file with Script Tag'),
                        'name' => 'CLASSY_BODY_CUSTOM_JS',
                        'label' => $this->l('JS In Body'),
                    ),
                ),
                'submit' => array(
                    'title' => $this->l('Save'),
    ...
    protected function getConfigFormValues()
    {
        return array(
            ...
            'CLASSY_HEAD_CUSTOM_JS' => html_entity_decode(Configuration::get('CLASSY_HEAD_CUSTOM_JS','')),
            'CLASSY_BODY_CUSTOM_JS' => html_entity_decode(Configuration::get('CLASSY_BODY_CUSTOM_JS','')),
        );
    }
    ...
    protected function postProcess()
    {
        ...
        foreach (array_keys($form_values) as $key) {
            ...
            if( 'CLASSY_HEAD_CUSTOM_JS' == $key || 'CLASSY_BODY_CUSTOM_JS' == $key ) {
                Configuration::updateValue($key, htmlentities($val));
            } else {
    ...
    */
    ...
    public function hookDisplayHeader()
    {
        ...
        if($jsenable){
            ...
            $entity_decode = html_entity_decode(Configuration::get('CLASSY_HEAD_CUSTOM_JS',''));
            $entity_decode = htmlspecialchars_decode( $entity_decode );

            // assign var to smarty
            $this->context->smarty->assign([
                'module_name' => $this->name,
                'classy_head_js' => $entity_decode,

            ]);

            return $this->display(__FILE__, 'views/templates/header/header.tpl');
        }
    ...
    public function hookdisplayBeforeBodyClosingTag() {
        ...
        if($jsenable){
            ...
            $entity_decode = html_entity_decode(Configuration::get('CLASSY_BODY_CUSTOM_JS',''));
            $entity_decode = htmlspecialchars_decode( $entity_decode );

            // assign var to smarty
            $this->context->smarty->assign([
                'module_name' => $this->name,
                'classy_footer_js' => $entity_decode,

            ]);

            return $this->display(__FILE__, 'views/templates/footer/footer.tpl');
        }
    }
