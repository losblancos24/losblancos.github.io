...
class blocktopdropdownmenu extends Module
{
    ...
    public function install($delete_params = false)
    {
        if (!parent::install() ||
        ...
        if ($delete_params) {
            if (!Configuration::updateGlobalValue('MOD_blocktopdropdownmenu_ITEMS', 'CAT3,CAT26') || !Configuration::updateGlobalValue('MOD_blocktopdropdownmenu_SEARCH', '1')) {
                return false;
            }
        }
    ...
    public function uninstall($delete_params = false)
    {
        if (!parent::uninstall()) {
            return false;
        }
        ...
        if (is_array($items) && count($items)) {
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    $shop_group_id = Shop::getGroupFromShop($shop_id);
                    $conf .= (string)($key > 1 ? ',' : '') . Configuration::get('MOD_blocktopdropdownmenu_ITEMS', null, $shop_group_id, $shop_id);
                }
            } else {
                $shop_id = (int)$shops[0];
                $shop_group_id = Shop::getGroupFromShop($shop_id);
                $conf = Configuration::get('MOD_blocktopdropdownmenu_ITEMS', null, $shop_group_id, $shop_id);
            }
    ...
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = MenuTopDropdownLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = MenuTopDropdownLinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
                        }
                ...
                case 'LNK':
                    ...
                    if (is_array($link)) {
                        if (count($link)) {
                            if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                                $default_language = Configuration::get('PS_LANG_DEFAULT');
                                $link = MenuTopDropdownLinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
                            }
        ...
        foreach ($categories as $key => $category) {
            ...
            if (isset($category['children']) && !empty($category['children'])) {
                ...
                if ((int)$category['level_depth'] > 0) {
                    ...
                    if (count(preg_grep('/^' . $category['id_category'] . '_thumb.jpg/i', $files)) > 0) {
    ...
    public function hookdisplayHeader()
    {
        ...
        $this->context->controller->addJS($this->_path . 'js/blocktopdropdownmenu.js');
        $this->context->controller->addCSS($this->_path . 'css/superfish-modified.css');
        $this->context->controller->addCSS($this->_path . 'css/blocktopdropdownmenu.css');
    }
    ...
    public function hookDisplayTop($param)
    {
        ...
        if (!$this->isCached('blocktopdropdownmenu.tpl', $this->getCacheId())) {
            ...
            $this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_blocktopdropdownmenu_SEARCH', null, $shop_group_id, $shop_id));
            $this->smarty->assign('MENU', $this->_menu);
            $this->smarty->assign('this_path', $this->_path);
        }

        $html = $this->display(__FILE__, 'blocktopdropdownmenu.tpl', $this->getCacheId());
        return $html;
    }

    public function hookdisplayNavFullWidth($param)
    {
        $this->user_groups = ($this->context->customer->isLogged() ?
            $this->context->customer->getGroups() : array(Configuration::get('PS_UNIDENTIFIED_GROUP')));
        $this->page_name = Dispatcher::getInstance()->getController();
    ...
    {
        $this->_clearCache('blocktopdropdownmenu.tpl');
    }
        ...
        if (count($shops) == 1) {
            $fields_form = array(
                'form' => array(
                    ...
                    'submit' => array(
                        'name' => 'submitblocktopdropdownmenu',
                        'title' => $this->l('Save')
                    )
                ),
            );
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                ...
                'submit' => array(
                    'name' => 'submitblocktopdropdownmenuLinks',
                    'title' => $this->l('Add')
                )
            ),
        );
    ...
    public function renderChoicesSelect()
    {
        ...
        $links = MenuTopDropdownLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = MenuTopDropdownLinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
    ...
    public function getConfigFieldsValues()
    {
        ...
        foreach ($shops as $shop_id) {
            $shop_group_id = Shop::getGroupFromShop($shop_id);
            $is_search_on &= (bool)Configuration::get('MOD_blocktopdropdownmenu_SEARCH', null, $shop_group_id, $shop_id);
        }
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updatelinksmenutop')) {
            $link = MenuTopDropdownLinks::getLinkLang(Tools::getValue('id_linksmenutop'), (int)Shop::getContextShopID());
    ...
    public function renderList()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, MenuTopDropdownLinks::gets((int)$this->context->language->id, null, (int)$shop_id));
        }
...
class blocktopdropdownmenuUpdate extends blocktopdropdownmenu
{
